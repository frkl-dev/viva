...
use crate::defaults::{CONDA_BIN_DIRNAME, DEFAULT_CHANNELS, VivaGlobals};
...
fn get_env_path(env_name: &str, globals: &VivaGlobals) -> PathBuf {
...
impl VivaEnv {
    ...
    pub fn create<S: AsRef<str>, I: AsRef<[S]>>(env_name: &str, specs: Option<I>, channels: Option<I>, env_prefix: Option<&PathBuf>, globals: &VivaGlobals) -> anyhow::Result<Self> {

        let env_path= match env_prefix {
            Some(prefix) => prefix.clone(),
            None => get_env_path(env_name, globals),
        };

        let env_specs: Vec<String> = if let Some(specs) = specs {
            specs.as_ref().iter().map(|x| String::from(x.as_ref())).collect()
        } else {
            vec![]
        };
        let env_channels: Vec<String> = if let Some(chans) = channels {
            chans.as_ref().iter().map(|x| String::from(x.as_ref())).collect()
        } else {
    ...
    pub async fn create_command_in_env<S: AsRef<str>, I: AsRef<[S]>>(&self, cmd: I, env_check_strategy: EnvCheckStrategy, pkg_install_strategy: PkgInstallStrategy) -> Result<Command> {
    ...
    pub async fn run_command_in_env<S: AsRef<str>, I: AsRef<[S]>>(&self, cmd: I, env_check_strategy: EnvCheckStrategy, pkg_install_strategy: PkgInstallStrategy) -> Result<()> {

        let mut command = self.create_command_in_env(&cmd, env_check_strategy, pkg_install_strategy).await?;

        let child = command
            .stdout(Stdio::piped())
            .spawn()
            .expect(format!("Failed to spawn subprocess: {}", cmd.as_ref().iter().map(|s| s.as_ref()).collect::<Vec<&str>>().join(" ")).as_str());
...
mod tests {
    ...
    fn test_viva_env_from_str_with_spec_file() {
        ...
        let globals = VivaGlobals::create("tests", "test_org", "viva");

        let env_prefix: Option<&PathBuf> = None;

        let viva_env_orig = VivaEnv::create(env_name, Some(vec!(String::from("spec1"), String::from("spec2"))), Some(vec!(String::from("channel_1"), String::from("channel_2"))), env_prefix, &globals).unwrap();
        ...
        let viva_env = VivaEnv::create(env_name, specs, channels, env_prefix, &globals).unwrap();
